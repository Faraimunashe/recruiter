    ...
    <section class="py-26  mb-5">
        <div class="container px-4 mx-auto relative">
            <div class="max-w-4xl mx-auto text-center">
                <h1 class="text-2xl md:text-2xl font-extrabold font-heading mt-4 mb-4">Vacancies Applied For</h1>
                    ...
                    <div class="px-6 md:px-12 py-8 mb-8 bg-white border-3 border-indigo-900 rounded-2xl shadow-md">
                        <div class="md:flex justify-between items-center">
                            ...
                            @if ($item->status === "pending")
                                <h3 class="text-blue-600"> PENDING</h3>
                            @elseif ($item->status === "accepted")
                                <h3 class="text-green-600"> ACCEPTED</h3>
                                <h3 class="text-green-700">
                                    @php
                                        $int = \App\Models\Interview::where('application_id', $item->id)->first();
                                        echo "Interview on ".$int->interview_date." at ".$int->interview_time;
                                    @endphp
                                </h3>
                            @elseif ($item->status === "declined")
                                <h3 class="text-red-700"> DECLINED</h3>
                            @endif
                        </div>
